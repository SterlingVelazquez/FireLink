...
class MassSystems extends React.Component {
  ...
  async componentDidMount() {
    firebase.auth().onAuthStateChanged(user => {
      if (user) {
        ...
        document.getElementById("massSignin").style.display="none";
        document.getElementById("massSideview").style.display="inline-block";
        if (user.email === "sterlin.velazquez37@gmail.com" || user.email === "firelinkwebsite@gmail.com") {
            this.setState({admin:"block"});
        }
      }
    })
    this.setState({files: await database.getAllFiles("MassSystemFiles")})
  }; 
  ...
  async signOut() {
    ...
    document.getElementById("massSignin").style.display="inline-block";
    document.getElementById("massSideview").style.display="none";
  }
  ...
  async showFiles() {
    document.getElementById("fileUploader").addEventListener('change', async (event) => {
        ...
        await database.add("MassSystemFiles", event.target.files, this.state.email, this.state.uid, this.state.name)
        this.setState({files: await database.getAllFiles("MassSystemFiles")})
  ...
  async openLink(file) {
    await firebase.storage().ref("MassSystemFiles/" + file.ref).getDownloadURL().then((res) => {
  ...
  async eraseFile(event, file) {
    ...
    await database.erase("MassSystemFiles", file, this.state.uid)
    this.setState({files: await database.getAllFiles("MassSystemFiles")})
  }
      ...
      <div className="container">
        ...
        <div className="burgIntroDiv">
          <div className="burgIntroContainer">
            <p className="burgIntroHeader">Mass Notification Services</p>
            <p className="burgIntroSub">Turnkey Installations • Parts and Smarts • Test and Certification • Annual Inspections</p>
            <br/>
            <p className="burgIntroBody">Our Mass Notification Systems are carefully designed by a Professional Engineer with over 13 years 
          ...
          <div style={{display:"inline-block"}} onClick={e => this.signIn()} id="massSignin" className="burgSignIn">
              <p><b>Sign In</b> To Your Google Account To Upload Files</p>
          </div>

          <div style={{display:"none"}} id="massSideview" className="burgSideContainer">
